...
class GameData():    
    def __init__(self, n, method=lambda layout:np.array([.25,.25,.25,.25])):
        import numpy as np
        
        for i in range(n):
            self.run_game(method)
    ...
    def log_data(self, game, move):
        formatted_move = np.zeros(4)
        formatted_move[['w','s','a','d'].index(move)] = 1

        try:
            game.layouts = np.concatenate((game.layouts, game.old_layout.reshape(1,-1)))
            game.moves = np.concatenate((game.moves, formatted_move.reshape(1,-1)))
        except AttributeError:
            game.layouts = game.old_layout.reshape(1,-1)
            game.moves = formatted_move.reshape(1,-1)
    
    def run_game(self, method):
        ...
        while game.active:
            ...
            for move in self.weighted_shuffle(moves, method(game.layout)):
                try:
                    game.swipe(move)
                    self.log_data(game, move) # move was successful, save data
                except:
